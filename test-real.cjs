...
(async () => {
  ...
  // click 新北市
  await page.evaluate(() => {
     try { document.querySelector('a.hrcity[value="F"]').click(); } catch(e){}
  });
  await new Promise(r => setTimeout(r, 1000));
  
  const towns = await page.evaluate(() => {
     return Array.from(document.querySelectorAll('#p_town option')).map(o => o.value + ":" + o.innerText);
  });
  ...
  await page.evaluate(() => {
     try {
       const select = document.querySelector('#p_town');
       if (select) {
         select.value = "F01";
